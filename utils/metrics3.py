...
def serendipity_at_k_batch2(rank_indices, train_user_dict, test_user_dict, user_ids, k, device='cuda'):
    ...
    recommended_items_k = rank_indices[:, :k]
    

    item_positions = torch.arange(1, k + 1, device=device).to(torch.bfloat16)  
    p_i = ((n_total_items + 1 - item_positions) / n_total_items).to(torch.bfloat16)  # Shape: (k,)
    p_i = p_i.unsqueeze(0).expand(n_users, -1)  # (Shape: (n_users, k))
    ...
    for idx in range(n_users):
        ...
        rel_i[idx] = torch.isin(recommended_items_k[idx], test_items).to(torch.bfloat16)  #Check whether the item is in the test set.
    

    serendipity_sum = torch.sum(p_i * rel_i, dim=1)  # Shape: (n_users,)
    serendipity_scores = serendipity_sum / k  # Shape: (n_users,)
    return serendipity_scores.mean()  
